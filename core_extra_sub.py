...
def process_code(rawcode, text, code_replace_dict=None):
    ...
    if num_parts == 4:
        ...
        subcode = rawcode_sep[1]
        sub_subcode = rawcode_sep[2]
        sub2_subcode = rawcode_sep[3]
        
        if sub2_subcode.find(",") == -1:
            output_list.append((rawcode, text))
            return output_list

        else:
            return_list = []
            for item in strip_list(sub2_subcode.split(",")):
                code_concat = main_code + ": " + subcode + ": "+ sub_subcode + ": "+ item
                return_list.append((code_concat, text))
            return return_list
...
def json_to_df(comments_json_dict, doc_ids = None, code_replace_dict=None):
    ...
    codes_df = pd.DataFrame(columns=["code","subcode","sub_subcode", "sub2_subcode", "name", "text", "comment_id", "coder", "url"])
    ...
    for row, items in codes_df.iterrows():
        ...
        for i in code_split:
            if count == 0:
                ...
                codes_df.iloc[row]['sub2_subcode']=i
            else:
    ...
    codes_df = codes_df.set_index(['code', 'subcode', 'sub_subcode', 'sub2_subcode', 'name']).sort_index()
